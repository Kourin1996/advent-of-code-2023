...
fn calculate_total(input: &str) -> u32 {
    ...
    let mut numbers: HashMap<(i32, i32), Vec<u32>> = HashMap::new();

    for (y, line) in array.iter().enumerate() {
        let mut x: i32 = 0;
        let y = y as i32;

        while x < (line.len() as i32) {
            while x < (line.len() as i32) && !line[x as usize].is_numeric() {
                x += 1;
            }
            ...
            // check
            if left - 1 >= 0 && line[(left - 1) as usize] == '*' {
                star_x = left - 1;
                star_y = y;
                has_star = true;
            }
            if right + 1 < (line.len() as i32) && line[(right + 1) as usize] == '*' {
                star_x = right + 1;
                star_y = y;
                has_star = true;
            }
            if !has_star {
                for cursor in (left - 1)..=(right + 1) {
                    if cursor < 0 || cursor >= (line.len() as i32) {
                        continue;
                    }

                    // upper
                    if y - 1 >= 0 && array[(y - 1) as usize][cursor as usize] == '*' {
                        star_x = cursor;
                        star_y = y - 1;
                        has_star = true;
                    }
                    if y + 1 < (array.len() as i32)
                        && array[(y + 1) as usize][cursor as usize] == '*'
                    {
                        star_x = cursor;
                        star_y = y + 1;
                        has_star = true;
            ...
            if has_star {
                let s = &line[(left as usize)..=(right as usize)]
                    .iter()
                    .collect::<String>();
    ...
    for (key, value) in numbers.into_iter() {
        if value.len() == 2 {
...
const INPUT: &str = "..975..95..................717..........................................747................................................622..............
