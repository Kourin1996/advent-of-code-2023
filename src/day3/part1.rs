...
fn calculate_total(input: &str) -> u32 {
    ...
    for (y, line) in array.iter().enumerate() {
        ...
        while x < (line.len() as i32) {
            ...
            // check
            if left - 1 >= 0 {
                is_adjacent_to_symbol |= is_symbol(line[(left - 1) as usize]);
            }
            if right + 1 < (line.len() as i32) {
                is_adjacent_to_symbol |= is_symbol(line[(right + 1) as usize]);
            }
            for cursor in (left - 1)..=(right + 1) {
                ...
                // upper
                if y - 1 >= 0 {
                    is_adjacent_to_symbol |= is_symbol(array[(y - 1) as usize][cursor as usize]);
                }
