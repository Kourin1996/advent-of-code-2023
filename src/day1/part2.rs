pub fn solve() {
    let res: u32 = INPUT.lines().map(extract_two_digits).sum();
    println!("{}", res);
}

fn extract_two_digits(input: &str) -> u32 {
    let nums = parse(input);

    let (left, right) = (nums.first(), nums.last());
...
fn parse(input: &str) -> Vec<u32> {
    ...
    let mut rest = input;
    while rest.len() > 0 {
        let (maybe_num, substr) = next(rest);

        if maybe_num.is_some() {
            nums.push(maybe_num.unwrap());
        }
...
fn next(input: &str) -> (Option<u32>, &str) {
    let first = input.chars().nth(0).unwrap();

    if first.is_numeric() {
        return (first.to_digit(10), &input[1..]);
    }

    match first {
        // one
        'o' if input.starts_with("one") => (Some(1), &input[2..]),
        // two
        't' if input.starts_with("two") => (Some(2), &input[2..]),
        // three
        't' if input.starts_with("three") => (Some(3), &input[4..]),
        // four
        'f' if input.starts_with("four") => (Some(4), &input[4..]),
        // five
        'f' if input.starts_with("five") => (Some(5), &input[3..]),
        // six
        's' if input.starts_with("six") => (Some(6), &input[3..]),
        // seven
        's' if input.starts_with("seven") => (Some(7), &input[4..]),
        // eight
        'e' if input.starts_with("eight") => (Some(8), &input[4..]),
        // nine
        'n' if input.starts_with("nine") => (Some(9), &input[2..]),
        _ => (None, &input[1..]),
    }
}
...
#[cfg(test)]
mod tests {
    use super::extract_two_digits;

    #[test]
    fn example1() {
        assert_eq!(extract_two_digits("two1nine"), 29);
    }

    #[test]
    fn example2() {
        assert_eq!(extract_two_digits("eightwothree"), 83);
    }

    #[test]
    fn example3() {
        assert_eq!(extract_two_digits("abcone2threexyz"), 13);
    }

    #[test]
    fn example4() {
        assert_eq!(extract_two_digits("xtwone3four"), 24);
    }

    #[test]
    fn example5() {
        assert_eq!(extract_two_digits("4nineeightseven2"), 42);
    }

    #[test]
    fn example6() {
        assert_eq!(extract_two_digits("zoneight234"), 14);
    }

    #[test]
    fn example7() {
        assert_eq!(extract_two_digits("7pqrstsixteen"), 76);
    }
}

static INPUT: &str = "2qlljdqcbeight
eight47srvbfive
