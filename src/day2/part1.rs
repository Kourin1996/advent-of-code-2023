const MaxRedCubes: u32 = 12;
const MaxGreenCubes: u32 = 13;
const MaxBlueCubes: u32 = 14;

pub fn solve() {
    let res: u32 = Input
        .lines()
        .filter_map(|input| {
            let (game, ok) = solve_game(input);
...
pub fn solve_game(input: &str) -> (u32, bool) {
    let mut res1 = input.split(':');
    let game = res1
        .next()
        .map(|game| {
            let mut res = game.split_whitespace();
            let index = res.nth(1);

            index.unwrap().parse::<u32>().unwrap()
        })
        .unwrap();
    ...
    let ok = res1
        ...
        .unwrap()
        .split(';')
        .all(|sub_game| solve_sub_game(sub_game.trim()));

    return (game, ok);
}

pub fn solve_sub_game(input: &str) -> bool {
    let mut red_count: u32 = 0;
    let mut green_count: u32 = 0;
    let mut blue_count: u32 = 0;

    input.split(',').map(|x| x.trim()).for_each(|value| {
        let mut res = value.split_whitespace();
        let num = res.next().unwrap().parse::<u32>().unwrap();
        let color = res.next().unwrap();
    ...
    red_count <= MaxRedCubes && green_count <= MaxGreenCubes && blue_count <= MaxBlueCubes
}
...
mod tests {
    ...
    fn example1() {
        assert_eq!(
            solve_game("Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green",),
            (1, true)
        );
    }
    ...
    fn example2() {
        assert_eq!(
            solve_game("Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue",),
            (2, true)
        );
    }
    ...
    fn example3() {
        assert_eq!(
            solve_game("Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red",),
            (3, false)
        );
    }
    ...
    fn example4() {
        assert_eq!(
            solve_game("Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red",),
            (4, false)
        );
    }
    ...
    fn example5() {
        assert_eq!(
            solve_game("Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green",),
            (5, true)
        );
    }
}

const Input: &str = "Game 1: 7 red, 14 blue; 2 blue, 3 red, 3 green; 4 green, 12 blue, 15 red; 3 green, 12 blue, 3 red; 11 red, 2 green
